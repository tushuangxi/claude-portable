#!/bin/bash
# ═══════════════════════════════════════════
# Pre-flight self-check for Claude Portable
...
preflight_check() {
    ...
    # 6. Port range available (17580-17589)
    local port_busy=0
    for p in $(seq 17580 17589); do
    ...
    if [ "$port_busy" -ge 10 ]; then
        echo "  [WARN]  Ports 17580-17589 all occupied"
        warnings=$((warnings + 1))
    fi
    ...
    local AUTH_FILE="$data_dir/.claude/auth.json"
    if [ -f "$AUTH_FILE" ]; then
